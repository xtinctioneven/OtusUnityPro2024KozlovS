...
public class DamageAllEffectHandler: BaseHandler<DamageAllEffectOld>
{
    ...
    protected override void OnHandleEvent(DamageAllEffectOld evt)
    {
        ...
        foreach (HeroEntity hero in targetHeroes)
        {
            ...
            if (hero == sourceHero)
            {
                // EventBus.RaiseEvent(new StatsResolveEvent(sourceHero, effectInteractionData));
                continue;
            }
