...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (!_world.IsEntityAlive (srcEntity)) { throw new Exception ("Cant touch destroyed src-entity."); }
            if (!_world.IsEntityAlive (dstEntity)) { throw new Exception ("Cant touch destroyed dest-entity."); }
#endif
            if (Has (srcEntity)) {
